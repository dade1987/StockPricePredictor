...
async function getOrderBookFutures(currency_pair_1) {
    let book_period = 10;

    let json_data = await binance.futuresDepth(currency_pair_1 + "USDT");

    let asks = json_data.asks;

    let bids = json_data.bids;

    let asks2 = new Object();
    let bids2 = new Object();

    asks.forEach((v) => {

        if (asks2[roundByNumber(v[0], book_period)] === undefined) {
            asks2[roundByNumber(v[0], book_period)] = new Object();
            asks2[roundByNumber(v[0], book_period)].volume = 0;
        }
        asks2[roundByNumber(v[0], book_period)].volume += parseFloat(v[1]);
    ...
    bids.forEach((v) => {
        ...
        if (bids2[roundByNumber(v[0], book_period)] === undefined) {
            bids2[roundByNumber(v[0], book_period)] = new Object();
            bids2[roundByNumber(v[0], book_period)].volume = 0;
        }
        //volume in crypto per arrivare a quel valore
        bids2[roundByNumber(v[0], book_period)].volume += parseFloat(v[1]);
    ...
    asks_vol = 0;
    first_asks_vol = 0;
    for (let key of Object.keys(asks2)) {
        if (i === 6) { break; }

        if (i === 0) {
            first_asks_vol = asks2[key].volume;
        }

        asks_vol += asks2[key].volume;
    ...
    bids_vol = 0;
    first_bids_vol = 0;
    for (let key of Object.keys(bids2).reverse()) {
        if (i === 6) { break; }

        if (i === 0) {
            first_bids_vol = bids2[key].volume;
        }

        bids_vol += bids2[key].volume;

        i++;
    }

    //console.log("FUTURES", asks2, bids2, asks_vol, bids_vol)
    ...
    return { trend: trend, status: json_data };
    ...
    return new Promise((resolve, reject) => {
        ...
        https.get(url.toLowerCase(), function(res) {
            ...
            res.on('end', function() {
                ...
                //console.log("SPOT", asks, bids, asks_vol, bids_vol)
